...
module Main (S: Mirage_types_lwt.STACKV4) = struct
  ...
  let iperf clock s src_ip src_port st buf =
    ...
    else if (Int32.compare id start_id) < 0 then (* a packet to close this trial *)
    begin
      ...
      let response = Cstruct.create 1470 in (* Preparation of a reply packet with C-based iperf compatibility *)
